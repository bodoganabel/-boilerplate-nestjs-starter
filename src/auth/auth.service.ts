...
import { IJwtPayload, User, UserDocument } from 'src/auth/user.schema';
...
export class AuthService {
  ...
  async signIn(
    ...
    if (user && (await bcrypt.compare(password, user.password))) {
      ...
      const permissionsDocument = await this.permissionModel.find(
        permissionQuery,
      );
      console.log('permissionsDocument:');
      console.log(permissionsDocument);

      const permissions = permissionsDocument.map(
        (permissionDocument) => permissionDocument.name,
      );
